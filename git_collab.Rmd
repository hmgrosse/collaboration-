...
```{r}
price_averages_for_animate <- t(state_price_averages)

prices_to_graph <- price_averages_for_animate %>%
...
```{r}
#Okay with the pivot thing I made above we now just have to figure out how to apply/use that in a map(:
states_map<- map_data("state") 
...
state_price_map <- price_averages_for_animate %>% 
  ...
  ggplot() + 
  geom_map(map = states_map,
           aes(map_id = row.names(RegionName),
               fill = `2008-03`,
               na.rm = TRUE)) +
