...
hurricane_sandy <- mean_price_year %>% 
  ...
  group_by(mean_price) %>% 
  arrange(mean_price) %>%
  select(RegionName == c(florida, georgia, south carolina, north carolina, virginia, maryland, deleware, pennsylvania, new jersey, new york, connceticut, rhode island, massacheusettes, new hampshire, maine))
  ggplot(aes(y = value, 
             x = mean_price, 
