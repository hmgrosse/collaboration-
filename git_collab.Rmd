...
#this graph is not working - because there is only one state? not sure how to make the line appear- 
...
hurricane_harvey <- price_by_month %>% 
  ...
  filter(RegionName == "texas",
   ...
   ggplot(aes(y = value, 
             ...
             #group = RegionName,
             color = RegionName)) +
  geom_line(fill = "orange", color = "orange") +
  ...
  labs(title = "Average State Sale Price by Month",
      ...
      color = "State",
      fill = "Average Sale Price by Month") +
  #geom_label_repel(data = harvey_labels,
                #   aes(label = "texas"),
                  # na.rm = TRUE) +
...
```
```{r}
#have to adjust the initial dataset 

city_data <- city_prices %>% 
  pivot_longer(cols = -c("StateName", "...1", "RegionID", "SizeRank"),
    names_to = "month_average") %>% 
  select(RegionName, month_average, value)

boston_bombing <- city_prices %>% 
  mutate(RegionName = str_to_lower(`RegionName`), 
         na.rm = TRUE) %>%
